define([], function () {
    ...
    function beforeLoad(context) {
        ...
        try {
            var functionCall = 'confirmSendEmail(\"' + invoiceNumber + '\",\"' + invoice_id + '\")';
            log.debug('functionCall', functionCall);
            var sendEmail = context.form.addButton({
                id: 'custpage_btn_confirmsendemail',
                label: 'Email Invoice',
                functionName: functionCall
            });
            context.form.clientScriptModulePath = 'SuiteScripts/sr_cli_emailinvoice_confirmation.js';
        } catch (error) {
